...
These are variants of integers, they are of less importance.
\begin{table}[h]
    \centering
    \begin{tabular}{|m{6em}|m{6em}|m{10em}|m{12em}|}
        \hline
...
A \index{Boolean} variable (\texttt{bool}) can be either \texttt{true} or \texttt{false}. There are three operators that we can perform on bools. Including AND (\&\&), OR (\textbar\textbar), NOT (!).
